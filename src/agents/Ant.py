...
class ComportTemporal(TimedBehaviour):
    ...
    def on_time(self):
        ...
        display_message(
            ...
            f"[{'Carregando' if self.agent.carrying else 'Procurando'}] Atual:: {self.agent.current_pos} Anterior:: {self.agent.last_pos}"
        )
...
class Ant(Agent):
    ...
    def choose_path(self):
        ...
        if self.carrying:
            self.find_path_when_carring_food()

        # Procurando comida
        else:
            self.find_path_to_food()

        Canvas().update_ant_position(self.last_pos, self.current_pos, Canvas().RED)

    def find_path_when_carring_food(self):
        to_home_path = []
        find_home = None
